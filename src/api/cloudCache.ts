...
import { compressFileOrFolder, tarGzExtension, uncompressTarBall } from "./helper/compress.ts";
...
const download = async ({
	...
	const { data: url, error: fetchError } = await tryCatch(getDownloadUrl(fileKey));
	if (fetchError) {
	...
	const { data: tarFile, error } = await tryCatch(downloadFile(url, targetPath));
	if (error) {
		apiLogger.log(`🌐 Failed to download file: ${error.message}`);
		return null;
	}
	const { data: apkPath, error: compressErr } = await tryCatch(uncompressTarBall(tarFile));
	if (compressErr) {
...
const upload = async ({ fileKey, artifactPath }: { fileKey: string; artifactPath: string }) => {
	...
	if (upload.error)
		return apiLogger.log(`🌐 Cache update: Failed to get upload link: ${upload.error}`);
