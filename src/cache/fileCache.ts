 ...
 * @param ignoredFile
 */
const cleanupCacheFiles = async (inputPath: string, maxAgeDays: number, ignoredFile?: string) => {
	...
	for (const file of files) {
		...
		if (statError) return logger.error(`Error getting stats for ${filePath}:1`, statError.message);
